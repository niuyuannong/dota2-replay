...
function processMatch(match, focusSlot) {
  ...
  const teamPlayers = players.filter(x => x.isRadiant === isRadiant);
  const enemyPlayers = players.filter(x => x.isRadiant !== isRadiant);
  const teamKills = teamPlayers.reduce((s,x) => s+(x.kills||0), 0);
  const teamNW = teamPlayers.reduce((s,x) => s+(x.networth||0), 0);
  const enemyNW = enemyPlayers.reduce((s,x) => s+(x.networth||0), 0);

  const dur = match.durationSeconds || 1;
  const cspm = (((p.numLastHits||0)+(p.numDenies||0))/dur*60).toFixed(1);
  const participation = teamKills > 0
    ? (((p.kills||0)+(p.assists||0))/teamKills*100).toFixed(0) : 0;

  return {
    heroName: getHeroName(p.heroId),
    won, duration: dur,
    kills: p.kills||0, deaths: p.deaths||0, assists: p.assists||0,
    kdaRatio: calcKDA(p.kills||0, p.deaths||0, p.assists||0),
    lastHits: p.numLastHits||0, denies: p.numDenies||0, cspm,
    gpm: p.goldPerMin||0, xpm: p.experiencePerMin||0,
    heroDamage: p.heroDamage||0,
    netWorth: p.networth||0, level: p.level||25,
    participation, teamNW, enemyNW, nwDiff: teamNW-enemyNW,
    matchId: match.id,
    allPlayers: players.map((x,i) => ({
      slot: i,
      heroName: getHeroName(x.heroId),
      kills:x.kills||0, deaths:x.deaths||0, assists:x.assists||0,
      gpm:x.goldPerMin||0, netWorth:x.networth||0,
      isTeam: x.isRadiant===isRadiant, isFocus: i===slot,
    }))
  };
}
...
export default function Home() {
  ...
  async function run() {
    if (!token.trim()) { setError("请填写 STRATZ Token"); return; }
    ...
    try {
      // 调用本地 API 代理（/api/match）
      const res = await fetch("/api/match", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ matchId: matchId.trim(), token: token.trim() })
      });
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .section-title{font-size:.7rem;color:var(--td);text-transform:uppercase;
        ...
        .foot a{color:var(--gd)}
        .note{font-size:.72rem;color:var(--td);margin-top:8px;line-height:1.6}
        .note a{color:var(--gd)}
      `}</style>
      ...
      <div className="wrap">
        <div className="hdr">
          ...
          <div className="hdr-s">STRATZ + Claude · MVP</div>
        ...
        <div className="panel">
          ...
          <div className="frow">
            <div className="fld fld-grow">
              <div className="flbl">STRATZ Token</div>
              <input type="password" value={token}
                onChange={e=>setToken(e.target.value)}
                placeholder="eyJhbGciOiJIUzI1NiIs..."/>
              <div className="note">
                登录 <a href="https://stratz.com/api" target="_blank" rel="noreferrer">stratz.com/api</a> 获取免费 Token
              </div>
            </div>
          </div>
          <div className="frow">
            <div className="fld fld-grow">
              ...
              <div className="note">
                登录 <a href="https://console.anthropic.com" target="_blank" rel="noreferrer">console.anthropic.com</a> → API Keys 获取
              </div>
            </div>
          </div>

          <div className="divider"/>
          <div className="section-title">比赛信息</div>
          <div className="frow">
            <div className="fld fld-grow">
          ...
          <button className="btn" onClick={run}
            disabled={busy||!matchId.trim()||!token.trim()||!anthropicKey.trim()}>
        ...
        {busy && (
          <div className="status">
            <div className="spin"/>
            <span>{phase==="fetching"?"正在从 STRATZ 拉取比赛数据…":"Claude 正在生成复盘报告…"}</span>
          </div>
        )}
        ...
        <div className="foot">
          数据来源：<a href="https://stratz.com" target="_blank" rel="noreferrer">STRATZ API</a> ·
